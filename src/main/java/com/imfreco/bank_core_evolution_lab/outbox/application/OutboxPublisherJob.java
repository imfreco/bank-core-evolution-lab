...
@Component
@ConditionalOnProperty(prefix = "bank.outbox.publisher", name = "enabled", havingValue = "true", matchIfMissing = true)
public class OutboxPublisherJob {
    ...
    public OutboxPublisherJob(OutboxEventRepository repository) {
    ...
    public void publishPendingEvents() {
        List<OutboxEvent> events = repository.findTop50ByStatusOrderByCreatedAtAsc(OutboxEventStatus.PENDING);
        for (OutboxEvent event : events) {
            try {
                // Simulacion: aqui se integraria Kafka, RabbitMQ, SNS/SQS u otro broker transaccionalmente desacoplado.
                log.info("Publishing outbox event id={} type={} aggregateId={}",
