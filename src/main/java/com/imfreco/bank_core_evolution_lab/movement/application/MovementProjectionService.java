...
public class MovementProjectionService {
    ...
    public MovementProjectionService(CustomerMovementViewRepository mongoRepository, AccountRepository accountRepository) {
    ...
    public Optional<CustomerMovementViewResponse> findView(UUID customerId) {
        try {
            ...
            log.warn("MongoDB movement view unavailable, falling back to SQL customerId={}", customerId);
    ...
    public void refreshCustomerMovements(UUID customerId, List<MovementResponse> sqlMovements) {
        try {
            List<CustomerMovementViewDocument.MovementItemDocument> items = sqlMovements.stream()
                    .map(item -> new CustomerMovementViewDocument.MovementItemDocument(
    ...
    private CustomerMovementViewResponse toResponse(CustomerMovementViewDocument document) {
        List<MovementResponse> movements = document.getMovements().stream()
                ...
                .toList();
        return new CustomerMovementViewResponse(UUID.fromString(document.getCustomerId()), movements,
                document.getUpdatedAt(), "MONGODB_READ_MODEL");
    }
}
